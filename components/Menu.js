...
export default function MenuComp() {
  ...
  const CurrentPage = (href) => {
    ...
    return `my-1 p-4 flex flex-row items-center gap-2 font-bold transition-all duration-200 ${
      isActive
        ? "bg-sidebar-foreground text-white shadow-inner border-r-4 border-[#A0CE66]"
        : "text-white hover:bg-sidebar-foreground hover:text-white/90"
    }`;
  };
  ...
  return (
    <div
      ...
      className="flex flex-col w-64 min-h-screen bg-sidebar border-r border-white/5 shadow-2xl"
    >
      {/* Logo Section */}
