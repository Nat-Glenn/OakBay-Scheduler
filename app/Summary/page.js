  ...
  return (
    <main className="flex h-dvh w-full bg-background relative overflow-hidden">
                  ...
                  {recentVisits.map((visit) => (
                    <TableRow key={visit.id} className="border-b last:border-0">
                      ...
                      <TableCell className="py-5 text-muted-foreground">
                        <span className={`text-sm px-3 py-1.5 rounded-full font-semibold ${visit.status === "Completed" ? "bg-[#a0ce66] text-black" : "bg-yellow-100 text-yellow-700"}`}>
                          {visit.status}
                        </span>
                      </TableCell>
                    </TableRow>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-base">
                    <span className="text-gray-600 font-medium">Medical Supplies</span>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-base">
                    <span className="text-gray-600 font-medium">Staff Salaries</span>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-base">
                    <span className="text-gray-600 font-medium">Facility Rent</span>
      ...
      <div className="fixed bottom-8 right-8 flex flex-col items-end z-50">
        {isChatOpen && (
          <Card className="w-96 mb-6 shadow-2xl border-none overflow-hidden p-0 animate-in slide-in-from-bottom-5 duration-300">
            ...
            <div className="p-4 border-t bg-white">
              <form className="flex gap-3 items-center" onSubmit={(e) => { e.preventDefault(); setQuery(""); }}>
                <Input placeholder="Type your message..." className="bg-slate-50 h-12 text-base border-slate-200 focus-visible:ring-[#002D58]" value={query} onChange={(e) => setQuery(e.target.value)} />
                <Button size="icon" className="h-12 w-12 bg-[#A0CE66] hover:bg-[#A0CE66]/80 text-white shrink-0 shadow-sm transition-colors">
                  <Send size={20} />
                </Button>
              </form>
            </div>
        ...
        <Button onClick={() => setIsChatOpen(!isChatOpen)} className={`h-16 w-16 rounded-full shadow-lg transition-all duration-300 ${isChatOpen ? "bg-slate-200 text-slate-600 rotate-90" : "bg-[#7AC242] text-white hover:bg-[#7AC242]/90 scale-110"}`}>
  ...
  return (
    <Card className="border-none shadow-sm hover:shadow-md transition-shadow">
      <CardContent className="pt-8 px-6 pb-6">
        ...
        <h3 className="text-md font-medium text-gray-500 uppercase tracking-wider text-xs">{title}</h3>
