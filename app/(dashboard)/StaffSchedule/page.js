...
export default function StaffSchedulePage() {
  ...
  const applyShiftToState = (data, payload) => {
    const cellKey = shiftMapKey(payload.providerId, payload.date);
    setShifts((prev) => {
      const withoutCell = prev.filter(
        (shift) => shiftMapKey(shift.providerId, shift.date) !== cellKey,
      );
      if (data?.working === false || !data?.id) {
        return withoutCell;
      }
      return [...withoutCell, data];
    });
  };
  ...
  const handleSaveShift = async (payload) => {
    ...
    try {
      ...
      applyShiftToState(data, payload);
              ...
              <tbody>
                {practitioners.map((provider) => (
                  <tr key={provider.id} className="border-b border-border/60">
                    ...
                    {dayNumbers.map((day) => {
                      const dateIso = dateIsoForDay(day);
                      const open = isClinicOpenOnDate(dateIso);
                      ...
                      const hoverTitle = open
                        ? formatShiftHoverTitle(
                            shift,
                            provider.name,
                            dateLabel,
                          )
                        : `${provider.name} — ${dateLabel}: Closed (no office hours)`;

                      return (
                        <td key={day} className="p-0.5">
                          <button
                            type="button"
                            disabled={!isAdmin || !open}
                            title={hoverTitle}
                            aria-label={hoverTitle}
                            onClick={() => open && openCell(provider, day)}
                            className={`flex h-9 w-full flex-col items-center justify-center rounded-md border text-[10px] leading-tight transition-colors ${
                              !open
                                ? "cursor-default border-transparent bg-muted/15 text-muted-foreground/70"
                                : working
                              ...
                              isAdmin && open ? "cursor-pointer" : "cursor-default"
                            }`}
                          >
                            {!open ? (
